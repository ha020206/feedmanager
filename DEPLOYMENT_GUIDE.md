...
2. **다음 Secrets 추가** (New repository secret 클릭). **값은 본인 프로젝트의 실제 키/설정으로 채우세요. 코드나 문서에 키를 붙여넣지 마세요.**
   
   **Gemini API:**
   - Name: `GEMINI_API_KEY`
   - Value: (본인 키 – [AI Studio](https://aistudio.google.com/apikey)에서 발급)
   
   **Firebase 설정:** (Firebase Console > 프로젝트 설정에서 확인)

...
// GitHub Pages 배포 시 환경변수는 GitHub Secrets를 사용하거나
// 빌드 시점에 주입해야 합니다.
const GEMINI_API_KEY = import.meta.env.VITE_GEMINI_API_KEY || "AIzaSyAVfxac7gUFsri0JF76FoKGQXaGHAFgfoM";
...
async function listAvailableModels() {
  try {
    ...
    if (!response.ok) {
      console.warn('모델 목록 조회 실패:', response.status);
      return null;
    }
    ...
    console.warn('모델 목록 조회 중 오류:', error);
    return null;
  }
}
 ...
 * 사용 가능한 모델로 콘텐츠 생성
 */
async function generateContentWithAvailableModel(prompt) {
  if (cachedModelName) {
    try {
      return await tryGenerateContent(cachedModelName, prompt);
    } catch (error) {
      console.log(`캐시된 모델 ${cachedModelName} 실패, 새 모델 찾는 중...`);
  ...
  try {
    ...
    if (availableModels && availableModels.length > 0) {
      modelsToTry = [...availableModels, ...DEFAULT_MODELS.filter(m => !availableModels.includes(m))];
    }
  } catch (error) {
    console.warn('모델 목록 조회 실패, 기본 모델 목록 사용:', error);
  }

  const errors = [];
  for (const modelName of modelsToTry) {
    try {
      const response = await tryGenerateContent(modelName, prompt);
      cachedModelName = modelName;
      console.log(`✅ 사용 가능한 모델 발견: ${modelName}`);
      return response;
    } catch (error) {
      const errorMsg = error.message || error.toString();
      errors.push(`${modelName}: ${errorMsg.substring(0, 150)}`);
      continue;
    }
  }
  ...
  throw new Error(
    ...
    `시도한 모델들 (일부):\n${errorSummary}\n\n` +
    `설정 방법: https://console.cloud.google.com/apis/library/generativelanguage.googleapis.com`
  );
}
...
`;
  ...
  try {
    const response = await generateContentWithAvailableModel(prompt);
...
`;
  ...
  return generateContentWithAvailableModel(prompt)
    .then(response => {

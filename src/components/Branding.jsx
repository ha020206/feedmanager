...
export default function Branding({ userData, onComplete }) {
  ...
  const isLoadingRef = useRef(false); // 중복 호출 방지 플래그
  ...
  // 자동 로고 생성 완전히 제거 - 사용자가 버튼을 눌러야만 생성
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      console.error('브랜딩 생성 오류:', error);
      setError(error.message || '브랜딩 생성 중 오류가 발생했습니다.');
      // 에러 발생 시에도 기본값으로 진행
      setRecommendations({
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <button
            onClick={() => {
              isLoadingRef.current = false; // 플래그 리셋
              loadRecommendations();
            }}
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <p className="text-gray-600 mb-8">마음에 드는 조합을 선택해주세요</p>
